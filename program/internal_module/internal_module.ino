...
void analyzeIncomingPacket(float* packet){
  ...
  if(packet[0] != MODULE_ID::INTERNAL_MODULE_ID) return; //Проверка, что данный пакет предназначен текущему модулю
...
void fillHeaderAndTailServicePacket(float* packet){
  ...
  packet[1] = MODULE_ID::INTERNAL_MODULE_ID;
...
void fillServicePacketSMS(float* packet){
  ...
  packet[4] = MODULE_ID::INTERNAL_MODULE_ID;
...
void fillDataPacket(float** dataArray){
  ...
  dataArray[0][1] = MODULE_ID::INTERNAL_MODULE_ID;
